...
class IbkrAdapterFetchTests(unittest.TestCase):
    ...
    def test_fetch_feed_payload_passes_debug_dir_as_path_to_client_factory(self) -> None:
        created_debug_dirs: list[Path | None] = []

        class RecordingClient(FakeIbkrClient):
            def __init__(self, debug_raw_xml_dir=None):
                super().__init__()
                created_debug_dirs.append(debug_raw_xml_dir)
        ...
        self.assertEqual(len(created_debug_dirs), 1)
        self.assertIsInstance(created_debug_dirs[0], Path)
        self.assertEqual(str(created_debug_dirs[0]), "scratch/debug-xml")

    def test_fetch_feed_payload_passes_none_debug_dir_to_client_factory(self) -> None:
        created_debug_dirs: list[Path | None] = []
        ...
        self.assertEqual(created_debug_dirs, [None])

    def test_fetch_feed_payload_closes_factory_created_client(self) -> None:
        close_calls: list[str] = []

        class CloseableRecordingClient(FakeIbkrClient):
            def __init__(self, debug_raw_xml_dir=None):
                super().__init__()
        ...
        adapter = IbkrFlexWebServiceAdapter(client_factory=CloseableRecordingClient)
        connection = IntegrationConnectionContext(
            connection_id="connection-uuid",
            integration_key="ibkr_flex_ws",
            brokerage_code="IBKR",
    ...
    def test_fetch_feed_payload_does_not_close_injected_client(self) -> None:
        close_calls: list[str] = []

        class CloseableFakeClient(FakeIbkrClient):
            def close(self) -> None:
                close_calls.append("closed")
